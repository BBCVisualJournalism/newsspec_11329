define(['lib/news_special/bootstrap', 'lib/news_special/share_tools/model', 'lib/news_special/share_tools/view'], function (news, SharetoolsModel, SharetoolsView) {
    ...
    NSShareController.prototype = {
        ...
        _initialiseModule: function () {
            ...
            news.pubsub.on('ns:' + this.namespace + ':share:message', function (target) { self._updateMessage(target); });
            news.pubsub.on('ns:' + this.namespace + ':share:image', function (target) { self._updateImage(target); });

            news.pubsub.on('ns:' + this.namespace + ':share:setFacebookMessage', function (target) { self._updateFacebookMessage(target); });
            news.pubsub.on('ns:' + this.namespace + ':share:setTwitterMessage', function (target) { self._updateTwitterMessage(target); });
            news.pubsub.on('ns:' + this.namespace + ':share:setEmailMessage', function (target) { self._updateEmailMessage(target); });
